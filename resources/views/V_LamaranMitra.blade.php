    ...
    <div class="page-breadcrumb bg-white">
        <div class="row align-items-center">
            <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                <h4 class="page-title text-uppercase font-medium font-14">Lamaran </h4>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table no-wrap">
                            ...
                            <tr>
                                ...
                                @if(Auth::user()->role==2)
                                    <th class="border-top-0">Nama Pelamar</th>
                            ...
                            <tbody>
                            @foreach($lamarans as $m)
                                <tr>
                                    <td>{{$m->id}}</td>
                                    <td>{{$m->lowongan->nama_lowongan}}</td>
                                    @if(Auth::user()->role==2)
                                        <td>{{$m->pelamar->user->name}}</td>
                                        <td>{{$m->statusPembelian->status}}</td>
                                    @endif
...
{{--                                    <td class="txt-oflo">{{$m->email}}</td>--}}
                                    <td class="txt-oflo">
                                        <a href="{{route('lihatlamaran',$m->id)}}" class="btn btn-primary">Detail</a>
                                    </td>
                                </tr>
                            @endforeach
